...
uncertainty <-
  read.csv("/Users/hampus/projects/equivariant-posteriors/experiments/lyapunov2/uncertainty_mnist.csv")
...
ggsave("uq_mnist_2.pdf")

ftle_xy <- (ggplot2::ggplot(uncertainty, aes(x=x, y=y, label=label))
 + geom_text(size=3, alpha=0.5, aes(color=lambda))) 
ggsave("ftle_xy.pdf", ftle_xy)
...
(ftle_mi_lambda <- ggplot2::ggplot(uncertainty, aes(x=lambda, y=MI))
  ...
  + scale_y_continuous(trans="log2"))
ggsave("lambda_MI.pdf", ftle_mi_lambda)
