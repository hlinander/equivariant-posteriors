...
def create_configs():
    ...
    epochs = _get_epochs()
    print(
        f"Sweep: {n_variants} climate model(s) × {n_seeds} seeds × 2 lrs = "
        f"{n_variants * n_seeds * 2} jobs, each evaluating {len(epochs)} epochs sequentially"
    )
...
def run(config):
    ...
    sample_run = base_create_config(ensemble_id=seed, climate_model_idx=climate_model_idx, lr=lr)
    ...
    print(f"=== [{config_name}] Evaluating {model_name} (idx={climate_model_idx}), seed={seed}, lr={lr}, epochs={epochs} ===")

    curried = lambda ensemble_id, **kw: base_create_config(
        ensemble_id=ensemble_id, climate_model_idx=climate_model_idx, lr=lr, **kw
    )
