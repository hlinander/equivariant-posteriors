...
#import xarray as xr   # only needed in __main__ block
#import pandas as pd   # only needed in __main__ block
#from xskillscore import rmse  # only needed in __main__ block
from sklearn.linear_model import LinearRegression
# emcli2 imports — only needed in __main__ block, not when feeding data directly
...
def fit_linear_regression_global_global(
    ...
    Args:
        data_dir str: Path to directory with input and target data. See
            ...
            data format. Ignored if input_array and target_array are provided.
        plot bool: if True, plots training data and fit.
        input_array np.array: Optional array of shape (n_samples,) or
            (n_samples, in_channels) to use instead of loading from disk.
        target_array np.array: Optional array of shape (n_samples,) or
            (n_samples, out_channels) to use instead of loading from disk.
    Returns:
    ...
    if input_array is not None and target_array is not None:
        # Use arrays passed directly — no disk I/O needed
        input_lr  = np.reshape(input_array,  (input_array.shape[0],  -1)) if input_array.ndim  > 1 else input_array[:,  None]
        target_lr = np.reshape(target_array, (target_array.shape[0], -1)) if target_array.ndim > 1 else target_array[:, None]
    else:
...
def predict_linear_regression_global_global(model,
    ...
    Args:
        ...
        data_dir str: Path to directory with input and target data. See
            emcli.dataset.interim_to_processed.interim_to_global_global for
            data format. Ignored if input_array is provided.
        plot bool: if True, plots test data and fit.
        input_array np.array: Optional array of shape (n_samples,) or
            (n_samples, in_channels) to use instead of loading from disk.
        target_array np.array: Optional array of shape (n_samples,) or
            (n_samples, out_channels) used only for plotting. Can be None.
    Returns
        preds np.array(n_samples, out_channels): predictions
    """
    if input_array is not None:
        # Use arrays passed directly — no disk I/O needed
        input_test_lr  = np.reshape(input_array, (input_array.shape[0], -1)) if input_array.ndim > 1 else input_array[:, None]
        target_test_lr = (np.reshape(target_array, (target_array.shape[0], -1)) if target_array.ndim > 1 else target_array[:, None]) if target_array is not None else None
    else:
        # Load processed data from disk
