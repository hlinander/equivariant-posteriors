...
class DummyMLP(BaseModel):
    """Intentionally simple MLP baseline for sanity-checking the evaluation pipeline.
    Flattens all spatial and temporal dimensions and passes through two linear layers.
    Produces poor results by design — no spatial awareness whatsoever.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if datamodule_config is not None:
            self.lon = datamodule_config.get("lon", lon)
            self.lat = datamodule_config.get("lat", lat)
            self.seq_len = datamodule_config.get("seq_len", seq_len)
            self.channels_last = datamodule_config.get("channels_last", channels_last)
            seq_to_seq = datamodule_config.get("seq_to_seq", seq_to_seq)
        else:
            self.lon = lon
            self.lat = lat
            self.seq_len = seq_len
        ...
        self.num_input_vars = len(in_var_ids)
        self.num_output_vars = len(out_var_ids)
        self.out_seq_len = self.seq_len if seq_to_seq else 1

        self.save_hyperparameters()

        in_features = self.seq_len * self.lon * self.lat * self.num_input_vars
        out_features = self.out_seq_len * self.lon * self.lat * self.num_output_vars
    ...
    def forward(self, x: Tensor) -> Tensor:
        # x: (batch, seq_len, lon, lat, in_vars) if channels_last
        #    (batch, seq_len, in_vars, lon, lat) if not channels_last
        if not self.channels_last:
            x = x.permute(0, 1, 3, 4, 2)  # -> channels_last

        batch_size = x.shape[0]
        x = x.reshape(batch_size, -1)
        x = self.model(x)
        x = x.reshape(batch_size, self.out_seq_len, self.lon, self.lat, self.num_output_vars)
