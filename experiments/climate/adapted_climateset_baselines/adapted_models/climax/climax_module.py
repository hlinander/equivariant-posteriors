...
import numpy as np
import torch
from .tokenized_vit_continuous import TokenizedViTContinuous
...
class ClimaXConfig:
    ...
    in_vars: List[str] = field(default_factory=lambda: ["CO2", "SO2", "CH4", "BC"])
    out_vars: List[str] = field(default_factory=lambda: ["pr", "tas"])  # is default vars for climate modelling
    ...
    # region_info: not supported in adapted model  # TODO: maybe later we could actually include that

class ClimaX(nn.Module):
    def __init__(self, config: ClimaXConfig, data_spec: DataSpec, **kwargs):
    #     self,
    #     climate_modeling: bool = True,  # always True with us...
    #     time_history: int = 1,
    #     lon: int = 128,
    ...
    # ):
        ...
        self.out_seq_len = config.seq_len if config.seq_to_seq else 1

        out_vars = config.out_vars
        in_vars = config.in_vars
        seq_to_seq = config.seq_to_seq
        climate_modeling = config.climate_modeling
        time_history = config.time_history
        patch_size = config.patch_size
        drop_path = config.drop_path
        drop_rate = config.drop_rate
        learn_pos_emb = config.learn_pos_emb
        channel_agg = config.channel_agg
        ...
        no_time_aggregation = config.no_time_aggregation
    ...
    def forward(self, batch: dict) -> dict:
        ...
        lead_times = torch.zeros(x.shape[0]).to(x.device)  # zero lead times for climate modelling
        x = self.model(x, lead_times)
        if self.channels_last:
            x = x.permute((0, 1, 3, 4, 2))
        x = x.nan_to_num()
