...
def create_config(
    ensemble_id,
    epoch=200, # previous was 200, find previous runs
    batch_size=12,
    climate_model_idx=0,
    lr=2e-4 ,
    ...
    def loss_fn_w_weights(output, batch):
        pred = output["logits_output"]
        target = batch["target"]
        if pr_variable_weighing == 0.5:
            return mse(pred, target)
        # Variables are in dim 1: [:,0]=tas, [:,1]=pr
    ...
    train_eval = TrainEval(
        train_metrics=[create_metric(loss_fn_w_weights)],
        validation_metrics=[create_metric(loss_fn_w_weights)],
...
if __name__ == "__main__":
    ...
    config = create_config(
        ensemble_id=seed_idx, epoch=200, climate_model_idx=climate_model_idx, pr_variable_weighing = 0.5
    )
