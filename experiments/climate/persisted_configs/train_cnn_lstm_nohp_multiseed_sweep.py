"""
Sweep file for training CNN-LSTM with multiple seeds on a single climate model.

Usage:
    # 10 seeds on climate model 0:
    CLIMATE_MODEL_IDX=0 python run_slurm_sweep.py \
        experiments/climate/persisted_configs/train_cnn_lstm_nohp_newlossfn_multiseed_sweep.py

    # Dry run to inspect the batch script:
    CLIMATE_MODEL_IDX=0 python run_slurm_sweep.py --dry-run \
        experiments/climate/persisted_configs/train_cnn_lstm_nohp_newlossfn_multiseed_sweep.py

    # Run locally (sequential, no SLURM):
    CLIMATE_MODEL_IDX=0 python run_slurm_sweep.py --run-local \
        experiments/climate/persisted_configs/train_cnn_lstm_nohp_newlossfn_multiseed_sweep.py
"""
