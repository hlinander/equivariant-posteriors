...
Uses correct lat-weighted RMSE loss (weights on lat axis, dim -2) instead of
the plain MSELoss used in train_unet_nohp_multiseed.py, bringing UNet's
training objective into line with ClimaX and CNN-LSTM.
...
class LLweighted_RMSELoss_Climax(torch.nn.Module):
    ...
    def forward(self, pred, y):
        # pred, y shape: (B, T, lat, lon)
        mse = self.mse(pred, y)
        lat_size = y.shape[-2]  # lat is axis -2; lon is axis -1
        ...
        weights = weights.reshape(1, 1, -1, 1).to(pred.device)  # (1, 1, lat, 1)
...
# ---- Model --------------------------------------------------------------
from experiments.climate.adapted_climateset_baselines.unet import (
    UNetConfig,
    UNet,
)
...
def create_config(ensemble_id, epoch=200, batch_size=12, climate_model_idx=0):
    ...
    def loss_fn(output, batch):
        pred = output["logits_output"]   # (B, T, C, lat, lon)
        target = batch["target"]         # (B, T, C, lat, lon)
        n_vars = pred.shape[2]
        loss = sum(
    ...
    # All seeds share the same data split; ensemble_id only affects model init
    random_seed  = 1
    val_fraction = 0.1
    seq_len      = 12
    seq_to_seq   = True
    ...
    train_config = TrainConfig(
        extra=dict(loss_variant="full_latfix"),
