...
from interim_to_processed import calculate_global_weighted_average
from climatebench import load_climatebench_data
from climatebench import input_for_training
from climatebench import output_for_training
from climatebench import normalize
from climatebench import load_climatebench_inputs_as_np_time_series
from climatebench import unnormalize
...
def load_mpi_data_as_xr(df, meta, climatology=None, m_member_subset=None, verbose=True,
    ...
    for scenario in scenarios:
        ...
        from mpi_esm1_2_lr import preprocess_data_var_mpi_esm1_2_lr
...
def preprocess_data_var_mpi_esm1_2_lr(data, data_var):
    ...
    data_annual = data[data_var].resample(time='YE').mean('time').to_dataset(promote_attrs=True)
...
def add_dtr_to_ensemble(dtr_annual_ens, df_realization, 
    ...
    # Compute annual mean
    dtr_annual = dtr.resample(time='YE').mean('time').to_dataset(name='dtr')
