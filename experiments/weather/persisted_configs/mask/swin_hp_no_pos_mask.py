...
def create_config(ensemble_id, epoch=200):
    ...
    train_config = TrainConfig(
        ...
        model_config=SwinHPPanguMaskConfig(
            base_pix=12,
            nside=NSIDE,
            dev_mode=False,
            depths=[2, 6, 6, 2],
    ...
    train_run = TrainRun(
        ...
        notes=dict(
            ...
            mask="uses topo, soil and land masks",
        ),
    )
    return train_run
...
if __name__ == "__main__":
    ...
    stats = deserialize_dataset_statistics(dl.dataset.config.nside).item()
    ...
    for idx, batch in enumerate(dl):
        ...
        save_and_register(f"{idx}_of_surface.npy", out_surface)
        save_and_register(f"{idx}_if_surface.npy", batch["input_surface"])
        save_and_register(f"{idx}_tf_surface.npy", target_surface)
        save_and_register(f"{idx}_of_upper.npy", out_upper)
