...
from .utils.shift_window_mask import (
...
class Pangu(nn.Module):
    ...
    def __init__(
        ...
        self.layer1 = BasicLayer(
            ...
            input_resolution=(8, math.ceil(lat / 4), math.ceil(lon / 4)),
            depth=2,
        ...
        self.downsample = DownSample(
            ...
            output_resolution=(
                8,
                math.ceil(math.ceil(lat / 4) / 2),
                math.ceil(math.ceil(lon / 4) / 2),
            ),
        )
        ...
        self.upsample = UpSample(
            embed_dim * 2,
            embed_dim,
            (8, math.ceil(math.ceil(lat / 4) / 2), math.ceil(math.ceil(lon / 4) / 2)),
            (8, math.ceil(lat / 4), math.ceil(lon / 4)),
        )
        ...
        self.patchrecovery2d = PatchRecovery2D((lat, lon), (4, 4), 2 * embed_dim, 4)
        self.patchrecovery3d = PatchRecovery3D(
            (13, lat, lon), (2, 4, 4), 2 * embed_dim, 5
        )
...
class Pangu_lite(nn.Module):
    ...
    def __init__(self, embed_dim=192, num_heads=(6, 12, 12, 6), window_size=(2, 6, 12)):
        ...
        self.patchrecovery3d = PatchRecovery3D(
            (13, 721, 1440), (2, 8, 8), 2 * embed_dim, 5
        )
    ...
    def forward(self, surface, surface_mask, upper_air):
        ...
        x = self.downsample(x)
        layer_out.append(
            ("downsample", x[0, :, 0].detach().reshape(Pl, 46, 90)[0, :, :])
        )
        x = self.layer2(x)
        layer_out.append(("layer2", x[0, :, 0].detach().reshape(Pl, 46, 90)[0, :, :]))
        x = self.layer3(x)
        layer_out.append(("layer3", x[0, :, 0].detach().reshape(Pl, 46, 90)[0, :, :]))
        x = self.upsample(x)
        layer_out.append(
            ("upsample", x[0, :, 0].detach().reshape(Pl, Lat, Lon)[0, :, :])
        )
