...
class Conv(torch.nn.Module):
    def __init__(self, config: ConvConfig, data_spec: DataSpec):
        super().__init__()
        self.config = config
        self.c1 = torch.nn.Conv2d(3, int(config.width * 96), kernel_size=3)
        self.c2 = torch.nn.Conv2d(
            int(config.width * 96), int(config.width * 96), kernel_size=3
        )
        self.c3 = torch.nn.Conv2d(
            int(config.width * 96), int(config.width * 96), kernel_size=3, stride=2
        )
        self.c4 = torch.nn.Conv2d(
            int(config.width * 96), int(config.width * 192), kernel_size=3
        )
        self.c5 = torch.nn.Conv2d(
            int(config.width * 192), int(config.width * 192), kernel_size=3
        )
        self.c6 = torch.nn.Conv2d(
            int(config.width * 192), int(config.width * 192), kernel_size=3, stride=2
        )
        self.c7 = torch.nn.Conv2d(
            int(config.width * 192), data_spec.output_shape[-1], kernel_size=1
        )
        ...
        torch.nn.init.normal_(
            self.c2.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 96) * 3 * 3))
        )
        torch.nn.init.normal_(self.c2.bias, 0.0, std=math.sqrt(1e-7))

        torch.nn.init.normal_(
            self.c3.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 96) * 3 * 3))
        )
        torch.nn.init.normal_(self.c3.bias, 0.0, std=math.sqrt(1e-7))

        torch.nn.init.normal_(
            self.c4.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 96) * 3 * 3))
        )
        torch.nn.init.normal_(self.c4.bias, 0.0, std=math.sqrt(1e-7))

        torch.nn.init.normal_(
            self.c5.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 192) * 3 * 3))
        )
        torch.nn.init.normal_(self.c5.bias, 0.0, std=math.sqrt(1e-7))

        torch.nn.init.normal_(
            self.c6.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 192) * 3 * 3))
        )
        torch.nn.init.normal_(self.c6.bias, 0.0, std=math.sqrt(1e-7))

        torch.nn.init.normal_(
            self.c7.weight, 0.0, std=math.sqrt(1.0 / (int(config.width * 192) * 3 * 3))
        )
